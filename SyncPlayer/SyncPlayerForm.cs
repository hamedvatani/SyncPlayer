...
namespace SyncPlayer
{
    ...
    {
        // private readonly List<AudioVisualPlayer> _players = new List<AudioVisualPlayer>();
        ...
        private void AddFiles(params string[] filenames)
        {
            ...
            foreach (string filename in filenames)
            {
                ...
                if (m < reader.TotalTime)
                    m = reader.TotalTime;
                reader.Close();
            }

            var max = (int)m.TotalSeconds;
            for (int i = 0; i < filenames.Length; i++)
            {
                var player = new AudioVisualPlayer { Filename = filenames[i] };
                player.SilenceBefore = max - player.AudioTime;
            ...
            Height = filenames.Length * 80 + 100;
        }
